...
(function () {
  ...
  async function runSearch(query) {
    ...
    if (r.item && r.score > 0) {
      ...
      // Wire the download button to a placeholder workbook file,
      // named after the real library file.
      if (downloadBtn) {
        if (downloadUrl) URL.revokeObjectURL(downloadUrl);
        const csv = [
          r.item.title,
          'Lyros Accounting — workbook template',
          '',
          'This is the unpopulated template. To run it with your Xero data each month,',
          'book a call at https://www.lyros.com.au/contact or email chris@lyros.com.au.',
          '',
          'Requested: "' + query.trim().replace(/"/g, "'") + '"',
        ].join('\n');
        downloadUrl = URL.createObjectURL(new Blob([csv], { type: 'text/csv' }));
        downloadBtn.href = downloadUrl;
        downloadBtn.setAttribute('download', r.item.filename.replace(/\.xlsx$/i, '') + '.csv');
      }
      setMailto(
        enquireLink,
